    ...
    if type(bars) is str:
        ...
        if hint['position'] == 'long':
            processed_hint = {
                ...
                'Net revenue': round(exit_price - entry_price - 2*slippage - commission,2),
                'symbol': hint['sym'],
                'hintTime': hint['time'],
                'hintTrigger': hint['price'],
                'hintDirection': hint['position'],
            ...
            processed_hint = {
                ...
                'exitTime': exit_bar['date'],
                'exitPrice': exit_price,
                'Net revenue':  round(entry_price - exit_price - 2*slippage - commission,2),
                'symbol': hint['sym'],
                'hintTime': hint['time'],
                'hintTrigger': hint['price'],
                'hintDirection': hint['position'],
        ...
        if hint['position'] == 'long':
            ...
            'Net revenue': round(bars[-10]['close'] - entry_price - 2*slippage - commission,2),
            'symbol': hint['sym'],
            'hintTime': hint['time'],
            'hintTrigger': hint['price'],
            'hintDirection': hint['position'],
            ...
            'exitTime': bars[-10]['date'],
            'exitPrice': bars[-10]['close'],
            'Net revenue': round(entry_price - bars[-10]['close'] - 2*slippage - commission,2),
            'symbol': hint['sym'],
            'hintTime': hint['time'],
            'hintTrigger': hint['price'],
            'hintDirection': hint['position'],
